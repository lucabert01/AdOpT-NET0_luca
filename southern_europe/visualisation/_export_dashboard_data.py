...
RUNS = {
    "technology_selection": "../Results_CCSchainOptimization/technology_selection/20260912150224_emissions_minC_technology_selection-1/optimization_results.h5",
...
def build_scenario(h5_rel: str) -> dict:
    ...
    for node_name in ccs_df["node"].unique():
        ...
        for (x, y), (_, real_row) in zip(positions, real_rows.iterrows()):
            sector = m._tech_sector(real_row["tech"])
            ...
            size = float(real_row["size"]) if "size" in real_row else float(candidates["size"].sum())
            total_emissions = float(real_row["total_emissions"])
            captured_annual = float(real_row["captured_annual"])
            utilization = (total_emissions / (size * HOURS_PER_YEAR)) if size > 0 else 0.0
            emitters.append({
                "node": node_name,
                "lon": round(x, 5),
                "lat": round(y, 5),
